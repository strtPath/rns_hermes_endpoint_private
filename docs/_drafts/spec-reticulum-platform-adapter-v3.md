...
default 30 minutes. The reasoning must carry over with it, because it is not arbitrary: RNS
drops destinations from other nodes' path tables over time, so a long-lived agent that never
re-announces becomes undiscoverable. `0` disables periodic re-announce, which is a legitimate
choice for a node that only ever receives.
...
## 14. Invariants the adapter must not break
